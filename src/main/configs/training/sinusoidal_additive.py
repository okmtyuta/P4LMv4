#!/usr/bin/env python3
"""
AdditiveSinusoidalPositionalEncoder を用いた学習設定。

目的:
- 加法的サイン波位置エンコードと単純平均集約の組み合わせで、
  既存ベースライン（位置なし）に対する改善有無を評価する。

比較対象:
- `src/main/configs/training/sinusoidal_multiplicative.py`
"""
...
from src.modules.data_process.positional_encoder import AdditiveSinusoidalPositionalEncoder
...
OUTPUT_H5 = Helper.ROOT / "outputs" / "experiments" / "plasma_lumos_1h" / "sinusoidal" / "additive" / "train_result.h5"

# 前処理パイプライン: 初期化 → 加法的PE → 平均集約
process_list = DataProcessList(
    iterable=[
        Initializer(),
        AdditiveSinusoidalPositionalEncoder(a=10000.0, b=1.0, gamma=0.0),
        Aggregator("mean"),
    ]
)
...
additive_config = TrainingRunnerConfig(
    input_hdf5_path=INPUT_H5,
    output_hdf5_path=OUTPUT_H5,
    dataset_name="plasma_lumos_1h_sinusoidal_additive",
